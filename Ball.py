...
class Ball():
    ...
    def move(self, direction):
        ...
        """
        if direction == 0:
            for i in range(self.location[1], self.location[1] + self.size[1]):
            ...
            for i in range(self.location[0], self.location[0] + self.size[0]):
                ...
                self.window.replace(self.color, (i, self.location[1] + self.size[1]))
    ...
    def movement(self, hit_player, blocks):
        ...
        if self.location[1] <= 0 or self.location[1] + self.size[1] >= self.window.size[1]:
            self.direction[1] = self.flip_number(self.direction[1])
        for block in blocks.blocks:
            self.change_direction(hit_player, block.hit_ball_on_side(locations), block.hit_ball_on_buttom(locations), block.hit_ball_on_top(locations))
